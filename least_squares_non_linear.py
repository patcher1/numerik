...
def solve_with_newton(xk, grad, hess, tol=10**-14, maxit=10000):
    x = newton(xk, grad, hess, tol, maxit)
    return x
...
'''
def solve_with_gauss_newton(xk, F, DF, equation_minimizer, tol=10**-14, maxit=10000):
    for i in range(maxit):
        sk = equation_minimizer(DF(xk), F(xk))   #Finds sk such that [DF(x)*sk - F(x)] is minimal / can't use linalg.solve since J(x) is not necessarily invertible
